...
Working directly on the orchestrator
####################################

You might occasionally want to work directly on the environment on the orchestrator host (e.g. when testing a module). The
``inmanta-workon`` command, installed by the RPM, provides that functionality. ``inmanta-workon myenvironment`` puts you in
the environment's project directory and activates its Python venv. If you don't know the name of the environment by heart,
