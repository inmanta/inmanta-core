...
as a result, the id of Router b is changed, router c is removed and router z is added to the model.
...
- A ``--delete-resource-set <resource-set-name>`` option is added. This option can be passed multiple times and indicates that the resource set with the given name should be removed from the model. This option can only be used in combination with the previous one.
...
* To delete shared resources (resources that do not belong to a resource set) a full compile is still needed as shared resources can not be removed by partial compiles. A solution to this is to perform scheduled full compiles to garbage-collect these shared resources.
