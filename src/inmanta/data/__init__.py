...
class Resource(BaseDocument):
    ...
    @classmethod
    async def copy_unchanged_resources_from_shared_resource_set(
        cls,
        environment: uuid.UUID,
        source_version: int,
        target_version: int,
        changed_shared_resources: list[ResourceIdStr],
        shared_resource_set_id: uuid.UUID,
        *,
        connection: Optional[asyncpg.connection.Connection] = None,
    ) -> None:
        """
        Copy the resources that belong to the shared resource set that did not have changes in this partial compile.
        To be executed only on partial compile iff there were resources in the shared resource set that had changes.

        :param environment: The environment of these resources.
        :param source_version: The version this partial compile is based on.
        :param target_version: The version this partial compile is creating. Currently necessary for populating the model of
            the resource, will be gone in a future update.
        :param changed_shared_resources: The ids of the resources on the shared resource set that suffered changes
            or were created in this partial compile.
        :param shared_resource_set_id: The id of the new revision of the shared resource set.
        :param connection: The connection to use.
        """
        query = f"""
            INSERT INTO {cls.table_name()}(
                ...
                LEFT JOIN public.resource_set_configuration_model rscm
                    ON r.environment=rscm.environment AND r.resource_set_id=rscm.resource_set_id
                WHERE r.environment=$1 AND rscm.model=$2 AND r.resource_set IS NULL AND NOT r.resource_id=ANY($4)
            )
        """
        await cls._execute_query(
            query,
            ...
            source_version,
            target_version,
            changed_shared_resources,
            shared_resource_set_id,
            connection=connection,
        )
