...
TABLES_TO_MIGRATE = [data.Project,
                     data.Environment,
                     data.Parameter,
                     data.Form,
                     data.FormRecord]
...
def main(mongo_host, mongo_port, mongo_database, pg_host, pg_port, pg_database, pg_username, pg_password):
    ...
    future = migrate_mongo_to_postgres(mongo_host, mongo_port, mongo_database, pg_host, pg_port, pg_database, pg_username,
                                       pg_password)
    loop.run_until_complete(future)
    loop.close()


async def migrate_mongo_to_postgres(mongo_host, mongo_port, mongo_database, pg_host, pg_port, pg_database, pg_username,
                                    pg_password):
    try:
        ...
        await data.connect(host=pg_host,
                           port=pg_port,
