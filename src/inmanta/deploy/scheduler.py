...
from inmanta.dto import Discrepancy, SchedulerStatusReport
...
class ResourceScheduler(TaskManager):
    ...
    async def _get_partial_model_versions_from_db(self, *, connection: asyncpg.connection.Connection) -> list[ModelVersion]:
        ...
        try:
            ...
            LOGGER.debug("Read latest model version %d", latest_version.version)
        except KeyError:
            return []
        else:
        ...
        async with self.state_update_manager.get_connection(connection=connection) as con, con.transaction():
            ...
            if first_version:
                ...
                await self.state_update_manager.mark_as_orphan(
                    self.environment,
                    orphaned_resources={rid: change.last_seen_version for rid, change in deleted.items()},
                    connection=con,
                )
