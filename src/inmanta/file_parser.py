...
class RequirementsTxtParser:
    ...
    def parse(cls, filename: str) -> list[Requirement]:
        ...
        """
        return [parse_canonical_requirement(requirement=r) for r in cls.parse_requirements_as_strs(filename)]
            ...
            for line in fd.readlines():
                if line_continuation_buffer:
                    ...
                    if not line.endswith("\\"):
                        if parse_canonical_requirement(requirement=line_continuation_buffer).name != removed_dependency:
                    ...
                    line_continuation_buffer = line
                elif parse_canonical_requirement(requirement=line).name != removed_dependency:
                    result += line
                else:
