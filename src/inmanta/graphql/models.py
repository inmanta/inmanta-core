...
from inmanta.const import NotificationSeverity
...
class Notification(Base):
    ...
    environment: Mapped[uuid.UUID] = mapped_column(ForeignKey("environment.id"))
...
class Environment(Base):
    ...
    __mapper_args__ = {"exclude_properties": ["project_relationship"]}
    id: Mapped[uuid.UUID] = mapped_column(UUID, primary_key=True)
    ...
    project: Mapped[uuid.UUID] = mapped_column(ForeignKey("project.id"))
    project_relationship: Mapped["Project"] = relationship(back_populates="environments")
...
class Project(Base):
    ...
    environments: Mapped[List["Environment"]] = relationship(
        back_populates="project_relationship", cascade="all, delete-orphan"
    )
