...
from the namespace it is loading into, published on a context variable for the duration of `load()`. The cache is controlled by the `compiler.cache` config option and can
