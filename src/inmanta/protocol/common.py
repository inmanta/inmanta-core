...
# Kept importable here for backwards compatibility; these live in inmanta.util so that the DTOs and other low level
# code can encode without depending on the protocol layer.
...
class AnnotatedHandler(Protocol):
    """Structural type for callables decorated with inmanta.protocol.decorators.handle.

    The handle decorator attaches the __protocol_method__, __protocol_mapping__ and
    __api_version__ attributes to the wrapped (async) function. This Protocol describes that
    augmented shape — and matches inmanta.types.HandlerType for the call signature — so
    call-site type checks don't have to fall back to Callable[..., Any].
    """

    __protocol_method__: Callable[..., object]
    __protocol_mapping__: dict[str, str]
    __api_version__: int | None

    def __call__(self, *args: object, **kwargs: object) -> types.AsyncioCoroutine[types.Apireturn]: ...
...
class CallTarget:
    ...
    def _get_endpoint_metadata(self) -> dict[str, list[tuple[str, AnnotatedHandler]]]:
        total_dict = {
            method_name: method
        ...
        methods: dict[str, list[tuple[str, AnnotatedHandler]]] = defaultdict(list)
        ...
        for method, handler_list in self._get_endpoint_metadata().items():
            for method_handlers in handler_list:
                ...
                for properties in MethodProperties.methods[method]:
                    ...
                    if handler.__api_version__ is None or properties.api_version == handler.__api_version__:
                        ...
                        url_map[url][properties.operation] = UrlMethod(properties, self, handler, method_handlers[0])
        return url_map
