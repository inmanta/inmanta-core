...
def environment_token_list(tid: uuid.UUID) -> list[model.Token]:
    ...
    the registry; idempotent (reproducible) tokens are not listed. Revoked tokens remain listed for
    auditing until they are cleaned up, as configured by
    :inmanta.config:option:`server.revoked-token-retention`.
