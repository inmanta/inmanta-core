    ...
    async def _call(
        ...
        with tracing.span(f"return_rpc.{method_properties.function.__name__}"):
            ...
            try:
                ...
                return common.Result(code=500, result={"message": "Call timed out"},client=self._transport_instance, method_properties=method_properties)

            return common.Result(code=return_value["code"], result=return_value["result"],client=self._transport_instance, method_properties=method_properties)
...
class LocalClient(TypedClient):
    ...
    async def _call(
        ...
        return self._process_response(method_properties, common.Result(code=response.status_code, result=response.body, client=self._transport_instance,method_properties=method_properties))
