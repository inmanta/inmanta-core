...
from inmanta.server import config as opt  # noqa: F401
    ...
    def add_static_content(
        ...
        self._handlers.append(
            routing.Rule(
                ...
                rest_server.StaticContentHandler,
                {
                    "transport": self,
                    "content": content,
...
class LocalClient(Client):
    ...
    def _get_op_mapping(self, url: str, method: str) -> common.UrlMethod:
        ...
        if url not in self._op_mapping:
            ...
            url_methods = self._op_mapping[url]

        if method in url_methods:
            return url_methods[method]
    ...
    async def _call[R: types.ReturnTypes](
        ...
        response = await rest.execute_call(self._server._transport, method_config, spec.body or {}, spec.headers)
        return common.Result(code=response.status_code, result=response.body, method_properties=method_properties)
