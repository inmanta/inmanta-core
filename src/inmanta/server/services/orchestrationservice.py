...
from inmanta import protocol as inmanta_protocol
from inmanta import resources as inmanta_resources
from inmanta import types
from inmanta.data import dataview
from inmanta.data import model as data_model
from inmanta.db import util as db_util
from inmanta.protocol import common, exceptions, methods, methods_v2
from inmanta.server import agentmanager
from inmanta.server import config as opt
from inmanta.server import diff, protocol, validate_filter
...
class CrossResourceSetDependencyError(Exception):
    def __init__(self, resource_id1: data_model.ResourceIdStr, resource_id2: data_model.ResourceIdStr) -> None:
        """
...
class ResourceSetValidator:
    def __init__(self, resources: collections.abc.Set[data.Resource]) -> None:
        self.resources = resources
        self.rid_to_resource_set = {res.resource_id: res.resource_set for res in self.resources}

    def _is_cross_resource_set_dependency(self, res: data.Resource, rid_dependency: data_model.ResourceIdStr) -> bool:
        """
...
class PartialUpdateMerger:
    ...
    def __init__(
        ...
        base_version: int,
        version: int,
        rids_in_partial_compile: collections.abc.Set[data_model.ResourceIdStr],
        updated_resource_sets: collections.abc.Set[str],
        deleted_resource_sets: collections.abc.Set[str],
        updated_and_shared_resources_old: collections.abc.Mapping[data_model.ResourceIdStr, data.Resource],
        rids_deleted_resource_sets: collections.abc.Set[data_model.ResourceIdStr],
    ) -> None:
        """
        ...
        self.non_shared_resources_in_partial_update_old: collections.abc.Mapping[data_model.ResourceIdStr, data.Resource] = {
            rid: r for rid, r in self.updated_and_shared_resources_old.items() if r.resource_set is not None
        }
        self.shared_resources_old: collections.abc.Mapping[data_model.ResourceIdStr, data.Resource] = {
    ...
    async def create(
        ...
        updated_resource_sets: collections.abc.Set[str],
        deleted_resource_sets: collections.abc.Set[str],
        connection: typing.Optional[asyncpg.Connection] = None,
    ) -> "PartialUpdateMerger":
        """
    ...
    def merge_updated_and_shared_resources(
        self, updated_and_shared_resources: collections.abc.Sequence[data.Resource]
    ) -> dict[data_model.ResourceIdStr, data.Resource]:
        """
         Separates named resource sets from the shared resource set and expands the shared set with the shared resources in
         the previous data_model.version.
    ...
    def _validate_constraints(
        self, new_updated_and_shared_resources: collections.abc.Mapping[data_model.ResourceIdStr, data.Resource]
    ) -> None:
        """
        ...
        for res_id, res in new_updated_and_shared_resources.items():
            ...
            try:
                ...
                raise exceptions.BadRequest(e.get_error_message())

    def _merge_shared_resources(
        self, shared_resources_new: dict[data_model.ResourceIdStr, data.Resource]
    ) -> collections.abc.Sequence[data.Resource]:
        """
    ...
    def _should_keep_dependency_old_shared_resources(self, rid_dependency: data_model.ResourceIdStr) -> bool:
        """
    ...
    def _merge_dependencies_shared_resource(
        self,
        old_deps: collections.abc.Sequence[data_model.ResourceIdStr],
        new_deps: collections.abc.Sequence[data_model.ResourceIdStr],
    ) -> collections.abc.Sequence[data_model.ResourceIdStr]:
        """
        ...
        """
        old_deps_cleaned: collections.abc.Set[data_model.ResourceIdStr] = {
    ...
    async def merge_unknowns(
        self, unknowns_in_partial_compile: collections.abc.Sequence[data.UnknownParameter]
    ) -> collections.abc.Sequence[data.UnknownParameter]:
        """
...
class OrchestrationService(protocol.ServerSlice):
    ...
    def __init__(self) -> None:
        super().__init__(inmanta.server.SLICE_ORCHESTRATION)

    def get_dependencies(self) -> list[str]:
        return [inmanta.server.SLICE_RESOURCE, inmanta.server.SLICE_AGENT_MANAGER, inmanta.server.SLICE_DATABASE]

    def get_depended_by(self) -> list[str]:
        return [inmanta.server.SLICE_TRANSPORT]

    async def prestart(self, server: protocol.Server) -> None:
        await super().prestart(server)
        self.agentmanager_service = typing.cast(agentmanager.AgentManager, server.get_slice(inmanta.server.SLICE_AGENT_MANAGER))
        self.autostarted_agent_manager = typing.cast(
            agentmanager.AutostartedAgentManager, server.get_slice(inmanta.server.SLICE_AUTOSTARTED_AGENT_MANAGER)
        )
        self.resource_service = typing.cast(resourceservice.ResourceService, server.get_slice(inmanta.server.SLICE_RESOURCE))
    ...
    async def _purge_versions(self) -> None:
        ...
        for env_item in envs:
            ...
            n_versions = await env_item.get(data.AVAILABLE_VERSIONS_TO_KEEP)
    ...
    @inmanta_protocol.handle(methods.list_versions, env="tid")
    async def list_version(
        self, env: data.Environment, start: typing.Optional[int] = None, limit: typing.Optional[int] = None
    ) -> types.Apireturn:
        if (start is None and limit is not None) or (limit is None and start is not None):
            raise exceptions.ServerError("Start and limit should always be set together.")

        if start is None or limit is None:
            start = 0
            limit = data.APILIMIT

        if limit > data.APILIMIT:
            raise exceptions.BadRequest(f"limit parameter can not exceed {data.APILIMIT}, got {limit}.")
    ...
    @inmanta_protocol.handle(methods.get_version, version_id="id", env="tid")
    async def get_version(
        self,
        env: data.Environment,
        version_id: int,
        include_logs: typing.Optional[bool] = None,
        log_filter: typing.Optional[str] = None,
        ...
        if limit is None:
            limit = data.APILIMIT
        elif limit > data.APILIMIT:
            raise exceptions.BadRequest(
                f"limit parameter can not exceed {data.APILIMIT}, got {limit}."
                f" To retrieve more entries, use /api/v2/resource_actions"
            )

        resources_out: list[types.JsonType] = []
        d = {"model": version, "resources": resources_out}
        resource_action_lookup: dict[data_model.ResourceVersionIdStr, list[data.ResourceAction]] = {}
    ...
    @inmanta_protocol.handle(methods.delete_version, version_id="id", env="tid")
    async def delete_version(self, env: data.Environment, version_id: int) -> types.Apireturn:
        version = await data.ConfigurationModel.get_version(env.id, version_id)
        if version is None:
            return 404, {"message": "The given configuration model does not exist yet."}
    ...
    @inmanta_protocol.handle(methods_v2.reserve_version, env="tid")
    async def reserve_version(self, env: data.Environment) -> int:
        return await env.get_next_version()

    @inmanta_protocol.handle(methods_v2.get_pip_config, env="tid")
    async def get_pip_config(
        self,
        env: data.Environment,
        version: int,
    ) -> typing.Optional[data_model.PipConfig]:
        version_object = await data.ConfigurationModel.get_version(env.id, version)
        if version_object is None:
            raise exceptions.NotFound(f"No configuration model with version {version} exists.")
    ...
    def _create_dao_resources_from_api_resources(
        self,
        env_id: uuid.UUID,
        resources: list[types.JsonType],
        resource_state: dict[
            data_model.ResourceIdStr, typing.Literal[const.ResourceState.available, const.ResourceState.undefined]
        ],
        resource_sets: dict[data_model.ResourceIdStr, typing.Optional[str]],
        set_version: typing.Optional[int] = None,
    ) -> dict[data_model.ResourceIdStr, data.Resource]:
        """
        ...
        all_requires: set[data_model.ResourceIdStr] = set()
        # list of all resources which have a cross agent dependency, as a tuple, (dependant,requires)
        cross_agent_dep: list[tuple[data.Resource, inmanta_resources.Id]] = []
        for res_dict in resources:
            # Verify that the version field and the version in the resource version id field match
            version_part_of_resource_id = inmanta_resources.Id.parse_id(res_dict["id"]).version
            ...
            if "requires" not in attributes:
                ...
                for req in attributes["requires"]:
                    rid = inmanta_resources.Id.parse_id(req)
    ...
    def _get_skipped_for_undeployable(
        self,
        resources: collections.abc.Sequence[data.Resource],
        undeployable_ids: collections.abc.Sequence[data_model.ResourceIdStr],
    ) -> collections.abc.Sequence[data_model.ResourceIdStr]:
        """
        ...
        provides_tree: dict[data_model.ResourceIdStr, list[data_model.ResourceIdStr]] = collections.defaultdict(list)
        for r in resources:
            if "requires" in r.attributes:
                for req in r.attributes["requires"]:
                    req_id = inmanta_resources.Id.parse_id(req)
                    provides_tree[req_id.resource_str()].append(r.resource_id)
        # Find skipped for undeployables
        work = list(undeployable_ids)
        skippeable: set[data_model.ResourceIdStr] = set()
        while len(work) > 0:
            current = work.pop()
    ...
    async def _put_version(
        ...
        env: data.Environment,
        version: int,
        rid_to_resource: dict[data_model.ResourceIdStr, data.Resource],
        unknowns: collections.abc.Sequence[data.UnknownParameter],
        version_info: typing.Optional[types.JsonType] = None,
        resource_sets: typing.Optional[dict[data_model.ResourceIdStr, typing.Optional[str]]] = None,
        partial_base_version: typing.Optional[int] = None,
        removed_resource_sets: typing.Optional[list[str]] = None,
        pip_config: typing.Optional[data_model.PipConfig] = None,
        *,
        ...
        if version <= 0:
            raise exceptions.BadRequest(f"The version number used ({version}) is not positive")
        ...
        for rid_name in resource_sets.keys():
            try:
                inmanta_resources.Id.parse_id(rid_name)
            except Exception as e:
                raise exceptions.BadRequest("Invalid resource id in resource set: %s" % str(e))
        ...
        async with connection.transaction():
            try:
                ...
                raise exceptions.ServerError("The given version is already defined. Versions should be unique.")

            all_ids: set[inmanta_resources.Id] = {inmanta_resources.Id.parse_id(rid, version) for rid in rid_to_resource.keys()}
            if is_partial_update:
                # Make mypy happy
                assert partial_base_version is not None
                # This dict maps a resource id to its resource set for unchanged resource sets.
                rids_unchanged_resource_sets: dict[data_model.ResourceIdStr, str] = (
                ...
                all_ids |= {inmanta_resources.Id.parse_id(rid, version) for rid in rids_unchanged_resource_sets.keys()}
            ...
            all_agents: collections.abc.Set[str] = {res.agent for res in rid_to_resource.values()}
    ...
    async def _trigger_auto_deploy(
        ...
        version: int,
        *,
        connection: typing.Optional[asyncpg.Connection],
    ) -> None:
        """
        ...
        if auto_deploy:
            ...
            push_on_auto_deploy = typing.cast(bool, await env.get(data.PUSH_ON_AUTO_DEPLOY))
            agent_trigger_method_on_autodeploy = typing.cast(str, await env.get(data.AGENT_TRIGGER_METHOD_ON_AUTO_DEPLOY))
    ...
    def _create_unknown_parameter_daos_from_api_unknowns(
        self, env_id: uuid.UUID, version: int, unknowns: typing.Optional[list[dict[str, types.PrimitiveTypes]]] = None
    ...
    @inmanta_protocol.handle(methods.put_version, env="tid")
    async def put_version(
        ...
        unknowns: list[dict[str, types.PrimitiveTypes]],
        version_info: types.JsonType,
        compiler_version: typing.Optional[str] = None,
        resource_sets: typing.Optional[dict[data_model.ResourceIdStr, typing.Optional[str]]] = None,
        pip_config: typing.Optional[data_model.PipConfig] = None,
    ) -> types.Apireturn:
        """
        ...
        if not compiler_version:
            raise exceptions.BadRequest("Older compiler versions are no longer supported, please update your compiler")
    ...
    @inmanta_protocol.handle(methods_v2.put_partial, env="tid")
    async def put_partial(
        self,
        env: data.Environment,
        resources: object,
        resource_state: typing.Optional[
            dict[data_model.ResourceIdStr, typing.Literal[const.ResourceState.available, const.ResourceState.undefined]]
        ] = None,
        unknowns: typing.Optional[list[dict[str, types.PrimitiveTypes]]] = None,
        version_info: typing.Optional[types.JsonType] = None,
        resource_sets: typing.Optional[dict[data_model.ResourceIdStr, typing.Optional[str]]] = None,
        removed_resource_sets: typing.Optional[list[str]] = None,
        pip_config: typing.Optional[data_model.PipConfig] = None,
    ) -> common.ReturnValue[int]:
        """
        :param unknowns: dict with the following structure
                    {
                     "resource": ResourceIdStr,
        ...
        try:
            pydantic.TypeAdapter(collections.abc.Sequence[data_model.ResourceMinimal]).validate_python(resources)
            ...
            # Make mypy happy
            resources = typing.cast(list[types.JsonType], resources)

        # validate resources before any side effects take place
        for r in resources:
            rid = inmanta_resources.Id.parse_id(r["id"])
            if rid.get_version() != 0:
                raise exceptions.BadRequest("Resources for partial export should not contain version information")
        ...
        async with data.Resource.get_connection() as con:
            async with con.transaction():
                ...
                current_versions: collections.abc.Sequence[data.ConfigurationModel] = (
                    await data.ConfigurationModel.get_versions(env.id, limit=1)
                )
                ...
                rid_to_resource: dict[data_model.ResourceIdStr, data.Resource] = self._create_dao_resources_from_api_resources(
                    env_id=env.id,
                    resources=resources,
                    resource_state=resource_state,
                    resource_sets=resource_sets,
                ...
                updated_resource_sets: collections.abc.Set[str] = {
                    sr_name for sr_name in resource_sets.values() if sr_name is not None
                }
            ...
            returnvalue: common.ReturnValue[int] = common.ReturnValue[int](200, response=version)
            try:
                await self._trigger_auto_deploy(env, version, connection=con)
            except exceptions.Conflict as e:
    ...
    @inmanta_protocol.handle(methods.release_version, version_id="id", env="tid")
    async def release_version(
        self,
        env: data.Environment,
        version_id: int,
        push: bool,
        agent_trigger_method: typing.Optional[const.AgentTriggerMethod] = None,
        *,
        connection: typing.Optional[asyncpg.Connection] = None,
    ) -> types.ReturnTupple:
        async with data.ConfigurationModel.get_connection(connection) as connection:
            async with connection.transaction():
                with db_util.ConnectionInTransaction(connection) as connection_holder:
                    ...
                    model = await data.ConfigurationModel.get_version_internal(
                        env.id, version_id, connection=connection, lock=data.RowLockMode.FOR_NO_KEY_UPDATE
                    )
                    if model is None:
                        return 404, {"message": "The request version does not exist."}

                    if model.released:
                        raise exceptions.Conflict(f"The version {version_id} on environment {env.id} is already released.")
                    ...
                    if undep:
                        undep_ids = [data_model.ResourceVersionIdStr(rid + ",v=%s" % version_id) for rid in undep]
                        ...
                        if skippable:
                            skippable_ids = [data_model.ResourceVersionIdStr(rid + ",v=%s" % version_id) for rid in skippable]
                            # not checking error conditions
                            await self.resource_service.resource_action_update(
                                env,
                                skippable_ids,
                    ...
                    if latest_version:
                        ...
                        increments: tuple[
                            collections.abc.Set[data_model.ResourceIdStr], collections.abc.Set[data_model.ResourceIdStr]
                    ...
                    if client is not None:
                        if not agent_trigger_method:
                            env_agent_trigger_method = await env.get(
                                data.ENVIRONMENT_AGENT_TRIGGER_METHOD, connection=connection
                            )
    ...
    async def deploy(
        ...
        if not allagents:
            return common.attach_warnings(404, {"message": "No agent could be reached"}, warnings)
        ...
        return common.attach_warnings(200, {"agents": sorted(list(present))}, warnings)

    @inmanta_protocol.handle(methods_v2.list_desired_state_versions, env="tid")
    async def desired_state_version_list(
        ...
        filter: typing.Optional[dict[str, list[str]]] = None,
        sort: str = "version.desc",
    ) -> common.ReturnValue[list[data_model.DesiredStateVersion]]:
        try:
            return await dataview.DesiredStateVersionView(
                environment=env,
                limit=limit,
                filter=filter,
                sort=sort,
            ...
            validate_filter.InvalidFilter,
            data.InvalidSort,
            data.InvalidQueryParameter,
            data.InvalidFieldNameException,
        ) as e:
            raise exceptions.BadRequest(e.message) from e

    @inmanta_protocol.handle(methods_v2.promote_desired_state_version, env="tid")
    async def promote_desired_state_version(
        self,
        env: data.Environment,
        version: int,
        trigger_method: typing.Optional[data_model.PromoteTriggerMethod] = None,
    ) -> None:
        if trigger_method == data_model.PromoteTriggerMethod.push_incremental_deploy:
            push = True
            agent_trigger_method = const.AgentTriggerMethod.push_incremental_deploy
        elif trigger_method == data_model.PromoteTriggerMethod.push_full_deploy:
            push = True
            agent_trigger_method = const.AgentTriggerMethod.push_full_deploy
        elif trigger_method == data_model.PromoteTriggerMethod.no_push:
            push = False
        ...
        if status_code != 200:
            raise exceptions.BaseHttpException(status_code, result["message"] if result else "")

    @inmanta_protocol.handle(methods_v2.get_diff_of_versions, env="tid")
    async def get_diff_of_versions(
        self,
        env: data.Environment,
        from_version: int,
        to_version: int,
    ) -> list[data_model.ResourceDiff]:
    ...
    def convert_resources(self, resources: list[data.Resource]) -> dict[data_model.ResourceIdStr, diff.Resource]:
    ...
    async def _check_version_exists(self, env: uuid.UUID, version: int) -> None:
        ...
        if not version_object:
            raise exceptions.NotFound(f"Version {version} not found")
