...
class OrchestrationService(protocol.ServerSlice):
    ...
    async def deploy(
        ...
        if agents is None:
            self.add_background_task(client.trigger(env.id, None, incremental_deploy, resource_ids))
        else:
            for agent in agents_to_call:
                self.add_background_task(client.trigger(env.id, agent, incremental_deploy, resource_ids))
