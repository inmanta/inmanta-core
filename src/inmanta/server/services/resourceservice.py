...
class ResourceService(protocol.ServerSlice):
    ...
    async def get_resource_actions(
        ...
        if limit and resource_action_dtos:
            ...
            next_params["last_timestamp"] = (
                resource_action_dtos[-1].started.astimezone(datetime.timezone.utc).replace(tzinfo=None)
            )
            ...
            previous_params["first_timestamp"] = (
                resource_action_dtos[0].started.astimezone(datetime.timezone.utc).replace(tzinfo=None)
            )
