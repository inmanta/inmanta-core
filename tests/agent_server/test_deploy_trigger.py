...
from inmanta_tests.utils import AsyncClosing, log_contains, log_doesnt_contain, retry_limited


@pytest.mark.asyncio(timeout=150)
async def test_deploy_trigger(server, client, resource_container, environment, caplog, no_agent_backoff):
    """
    ...
    async with AsyncClosing(get_agent(server, environment, "agent1", "agent5")):
        ...
        async def verify(result, a1=0, code=200, warnings=["Could not reach agents named [agent2,agent3]"], agents=["agent1"]):
            ...
            log_doesnt_contain(
                caplog, "agent", logging.INFO, f"Agent agent5 got a trigger to update in environment {environment}"
            )
        ...
        await verify_failed(
            result, 404, "No agent could be reached", warnings=["Model version 5 does not contain agents named [agent5]"]
        )
        ...
        await verify(
            ...
            a1=3,
            agents=["agent1"],
            warnings=["Could not reach agents named [agent2]", "Model version 5 does not contain agents named [agent5]"],
        )
