...
#set passphrase
# Root cert
openssl req -sha256 -new -x509 -days 1826 -key rootca.key -out rootca.crt --passin pass:test
...
openssl req -new -sha256 -key intermediate1.key -out intermediate1.csr --passin pass:test
...
openssl req -new -sha256 -key ../enduser-certs/server.key -out ../enduser-certs/server.csr -passin pass:test
