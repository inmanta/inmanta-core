...
@pytest.fixture
async def migrate_v202212010_to_v202212090(
    hard_clean_db, hard_clean_db_post, postgresql_client: Connection, server_config
) -> AsyncIterator[Callable[[], Awaitable[None]]]:
    """
    Returns a callable that performs a v202212010 database restore and migrates to v202212090.
    """
    # Get old tables
...
async def test_added_environment_metrics_tables(
    migrate_v202212010_to_v202212090: Callable[[], Awaitable[None]],
    ...
    assert "environment" not in (await get_columns_in_db_table("environmentmetricsgauge"))
    assert "environment" not in (await get_columns_in_db_table("environmentmetricstimer"))
    columns_pk = await get_primary_key_columns_in_db_table("environmentmetricsgauge")
    assert len(columns_pk) == 2
    assert "environment" not in columns_pk

    # Migrate DB schema
    await migrate_v202211230_to_v202212010()

    # The table is added to the database
    assert "environment" in (await get_columns_in_db_table("environmentmetricsgauge"))
    assert "environment" in (await get_columns_in_db_table("environmentmetricstimer"))
    columns_pk = await get_primary_key_columns_in_db_table("environmentmetricsgauge")
    assert len(columns_pk) == 3
    assert "environment" in columns_pk
