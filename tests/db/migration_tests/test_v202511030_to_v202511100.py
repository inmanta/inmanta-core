...
async def test_add_last_deploy_compliant_to_rps_table(
    ...
    deployed_rps = {rps for rps in all_rps if rps.last_deploy_result is DeployResult.DEPLOYED}
    failed_skipped_rps = {rps for rps in all_rps if rps.last_deploy_result in (DeployResult.FAILED, DeployResult.SKIPPED)}
