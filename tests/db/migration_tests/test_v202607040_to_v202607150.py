...
async def test_register_modules_per_model_version(
    ...
    # become the on disk install mode (see v202608070).
    assert await postgresql_client.fetchval("SELECT count(*) FROM public.inmanta_module WHERE install_mode != 'on_disk'") == 0
