...
from inmanta.env import LocalPackagePath, process_env, safe_parse_requirement, safe_parse_requirements
...
def test_module_update_with_v2_module(
    ...
    for module_name, versions in {
        ...
        for current_version in versions:
            ...
            module_from_template(
                ...
                new_requirements=(
                    [InmantaModuleRequirement(safe_parse_requirement(requirement="module2<3.0.0"))]
    ...
    module_from_template(
        ...
        new_requirements=[InmantaModuleRequirement(safe_parse_requirement(requirement="module2"))],
        install=False,
        publish_index=pip_index,
...
def test_module_update_dependencies(
    ...
    for v in ("1.0.0", "1.0.1", "2.0.0"):
        create_python_package(
            ...
            requirements=[safe_parse_requirement(requirement="c")],
            publish_index=index,
        )
    ...
    process_env.install_for_config(
        [safe_parse_requirement(requirement=req) for req in ("b==1.0.0", "c==1.0.0")],
        config=PipConfig(
            index_url=index.url,
    ...
    v1_module_from_template(
        ...
        new_name="my_mod",
        new_requirements=safe_parse_requirements(["a", "b~=1.0.0"]),
    )
