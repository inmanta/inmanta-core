...
async def test_compile_reports_filters(server, client, env_with_compile_reports):
    ...
    assert parse_timestamp(result.result["data"][0]["requested"]) == compile_requested_timestamps[4].astimezone(datetime.UTC)
    assert parse_timestamp(result.result["data"][1]["requested"]) == compile_requested_timestamps[5].astimezone(datetime.UTC)
    assert parse_timestamp(result.result["data"][2]["requested"]) == compile_requested_timestamps[6].astimezone(datetime.UTC)
