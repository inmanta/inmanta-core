...
def test_docstring_fallback_for_field_descriptions():
    """
    ...
    for type_name, strawberry_type in mapper.mapped_types.items():
        ...
        # Only check fields that were generated by the mapper (not custom resolvers)
        ...
        for f in type_def.fields:
            ...
            try:
                ...
                # Relationship property, not a column
                continue
            col_doc = col_obj.doc
...
async def test_graphql_field_descriptions(server, client):
    """
    Verify that the GraphQL schema exposes field-level documentation via GraphQL introspection.
    """
    introspection_query = """
    {
        __schema {
