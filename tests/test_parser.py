...
def test_pickle_reentrant_namespace_safe():
    ...
    before calling load() works correctly. Each load() publishes its own
    namespace for its duration, so they don't interfere with each other.
    """
    root_ns = Namespace("__root__")
    ns_a = Namespace("ns_a")
    ns_a.parent = root_ns
...
def test_pickle_concurrent_threads():
    ...
    Each thread resolves the namespace from its own context, so concurrent
