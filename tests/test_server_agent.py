...
@pytest.mark.asyncio(timeout=30)
async def test_push_full_deploy_with_deployment_timer_enabled(resource_container,
                                                              server,
                                                              client,
                                                              environment,
                                                              no_agent_backoff,
    ...
    config.Config.set("config", "agent-deploy-interval", "600")
    config.Config.set("config", "agent-deploy-splay-time", "5")
    config.Config.set("config", "agent-repair-interval", "0")
    agent_name = "agent1"
    myagent = agent.Agent(hostname="node1", environment=environment, agent_map={agent_name: "localhost"}, code_loader=False)
    myagent.add_end_point_name("agent1")
    ...
    def get_resources(version, value):
        return [{'key': 'key1',
                 'value': value,
                 'id': 'test::Wait[agent1,key=key1],v=%d' % version,
                 'send_event': False,
    ...
    # Initial deploy
    await _deploy_resources(client, environment, resources_version_1, version1, True)
    ...
    # Initial deploy
    await _deploy_resources(client, environment, resources_version_2, version2, True)
    await resource_container.wait_for_done_with_waiters(client, environment, version2)
